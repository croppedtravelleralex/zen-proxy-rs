...
| T7.3 | 7 | Unify node and transport registries | pool/transport | T7.1 | one node registry and one client cache are visible in `/admin/runtime` |
| T7.4 | 7 | Add sharded ready queues | pool dispatch | T7.3 | dispatch uses configurable shards and avoids one global ready-list lock |
| T7.5 | 7 | Reduce Redis budget hot-path overhead | pool budget | T7.4 | Redis budget reuses an instance-local connection and exposes mode |
