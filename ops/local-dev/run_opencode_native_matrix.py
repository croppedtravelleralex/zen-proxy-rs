...
CASES = [
    ("tide", r"D:\SelfMadeTool\Tide", "全面深入检查项目并详细汇报"),
    ("mirofish", r"D:\SelfMadeTool\MiroFish", "全面深入检查项目并详细汇报"),
    ("personal-cleanup", r"D:\SelfMadeTool\personal", "检查本机c、d盘可删除内容"),
    (
        "outlook-register",
...
def main() -> int:
    ...
    for public_model in selected_models:
        ...
        for case_id, cwd, prompt in selected_cases:
            print(
                f"=== run native model={public_model} opencode={opencode_model} case={case_id} ===",
                flush=True,
            )
            result = run_case(
                run_dir,
            ...
            print(
                f"done model={public_model} case={case_id} exit={result.exit_code} "
