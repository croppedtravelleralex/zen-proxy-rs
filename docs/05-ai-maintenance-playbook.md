...
- ClaudeCode 工具名大小写/别名：ClaudeCode 注册的是 `WebSearch`、`WebFetch`、`Task` 等真实工具名；如果上游返回 `web_search`、`web_fetch`、`task`，客户端可能不执行。free-model-client-rs 源码已补 canonicalization 回归，panda 是否生效要看部署 hash 和线上 smoke。
