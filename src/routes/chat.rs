...
pub async fn chat_handler(State(state): State<Arc<AppState>>, headers: axum::http::HeaderMap, body: String) -> Response {
    ...
    let req: ChatRequest = match serde_json::from_str(&body) { Ok(r) => r, Err(e) => return AppError::invalid_json(e.to_string()).into_response() };
    ...
    if !state.config.free_models.iter().any(|m| m == nm) { return AppError::invalid_model(req.model).into_response(); }
    match proxy::openai::handle_openai_chat(&state, req).await { Ok(r) => r, Err(e) => e.into_response() }
}
...
pub async fn messages_handler(State(state): State<Arc<AppState>>, headers: axum::http::HeaderMap, body: String) -> Response {
    ...
    let nm = req.model.strip_prefix("opencode/").unwrap_or(&req.model);
    if !state.config.free_models.iter().any(|m| m == nm) { return AppError::invalid_model(req.model).into_response(); }
    match proxy::anthropic::handle_anthropic_messages(&state, req).await { Ok(r) => r, Err(e) => e.into_response() }
}
